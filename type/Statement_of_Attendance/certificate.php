...
$sql2 = "SELECT fs.id, fs.duration, fsd.timefinish
           FROM {facetoface_sessions} fs
           JOIN {facetoface} f ON fs.facetoface = f.id AND f.course = $course->id
           JOIN {facetoface_sessions_dates} fsd ON fs.id = fsd.sessionid
           JOIN {facetoface_signups} fsi ON fs.id = fsi.sessionid
                AND fsi.userid = $USER->id
           JOIN {facetoface_signups_status} fss ON fsi.id = fss.signupid
                AND fss.statuscode = 100
                AND fss.superceded = 0
       ORDER BY fsd.timefinish DESC";
$session = $DB->get_record_sql($sql2);
