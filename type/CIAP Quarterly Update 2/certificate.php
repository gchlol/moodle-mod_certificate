...
if(!empty($reports)){
    ...
    foreach($reports as $report){
        ...
        if(!empty($actions)){
            ...
            foreach($actions as $action){
                ...
                if($actionstatus == "In Progress"){
                    ...
                    // If the DueDate was updated
                    $dueDateQuestion = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Due Date Amended Progress', 'deleted' => 'n'));
                    $dueDateAnswer = $DB->get_record('questionnaire_response_bool', array('response_id' => $updateid, 'question_id' => $dueDateQuestion->id));

                    if($dueDateAnswer->choice_id == 'y'){

                        // Question 7.
                        $question7 = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Amended Date Progress', 'deleted' => 'n'));
                        $answer7 = $DB->get_record('questionnaire_response_date', array('response_id' => $updateid, 'question_id' => $question7->id));
                    ...
                    // If the Person Responsible was updated
                    $personQuestion = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Person Amended Progress', 'deleted' => 'n'));
                    $personAnswer = $DB->get_record('questionnaire_response_bool', array('response_id' => $updateid, 'question_id' => $personQuestion->id));

                    if($personAnswer->choice_id == 'y'){

                        // Question 8.
                        $question8 = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Amended Person Progress', 'deleted' => 'n'));
                        $answer8 = $DB->get_record('questionnaire_response_text', array('response_id' => $updateid, 'question_id' => $question8->id));
                        $pdf->SetFont('helvetica', 'B', '14');
                ...
                // If the action is in Not Yet Started.
                    ...
                    // If the DueDate was updated
                    $dueDateQuestion = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Due Date Amended NotStarted', 'deleted' => 'n'));
                    $dueDateAnswer = $DB->get_record('questionnaire_response_bool', array('response_id' => $updateid, 'question_id' => $dueDateQuestion->id));

                    if($dueDateAnswer->choice_id == 'y'){

                        // Question 7.
                        $question7 = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Amended Date NotStarted', 'deleted' => 'n'));
                        $answer7 = $DB->get_record('questionnaire_response_date', array('response_id' => $updateid, 'question_id' => $question7->id));
                        $date = new DateTime($answer7->response);
                        $pdf->SetFont('helvetica', '', '14');
                        $pdf->writeHTML('<b>Implemented by:</b> <i>' . $date->format('d/m/Y') . '</i>');
                    ...
                    // If the Person Responsible was updated
                    $personQuestion = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Person Amended NotStarted', 'deleted' => 'n'));
                    $personAnswer = $DB->get_record('questionnaire_response_bool', array('response_id' => $updateid, 'question_id' => $personQuestion->id));

                    if($personAnswer->choice_id == 'y'){

                        // Question 8.
                        $question8 = $DB->get_record('questionnaire_question', array('surveyid' => $updatequestionnaireid, 'name' => 'Amended Person NotStarted', 'deleted' => 'n'));
                        $answer8 = $DB->get_record('questionnaire_response_text', array('response_id' => $updateid, 'question_id' => $question8->id));
                        $pdf->SetFont('helvetica', 'B', '14');
                        $pdf->writeHTML('Who will be responsible for implementing the action? <span style="font-weight: normal;"><i>' . $answer8->response . '</i></span>');
                    }
        ...
        // Get all actions related to the report.
        $newactions = $DB->get_records_sql($sql);//, ['reportname' => $reportname]);
